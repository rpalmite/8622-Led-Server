...
// NET DISCOVERER
void NetDiscoverer::initDiscoveryListening() {
  IPAddress localIP = WiFi.localIP();
	//DEBUG_MSG(1, F("RECEIVER Connected. Local IP"), localIP);

  //IPAddress mcastIP(MCAST_ADDRESS); // multicast address
	if ( !discovery.begin(mcastIP, mcastPort, localIP) ) { // join mcast group // MCAST_PORT
...
// TODO make senderACK a class member variable
void NetDiscoverer::listenForDiscovery(std::array<bool, 256> senderACK) {
