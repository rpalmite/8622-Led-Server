...
public:
  void initDiscoveryListening();
  void listenForDiscovery(std::array<bool, 256> senderACK);

private:
  NetDiscovery discovery;
  const IPAddress mcastIP{239, 1, 4, 77}; // multicast address
  const int mcastPort = 7245; // TODO make this const
};
#endif